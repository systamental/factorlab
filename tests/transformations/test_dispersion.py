...
def test_standard_deviation(binance_spot, btc_spot_prices, axis, window_type) -> None:
    ...
    actual = StandardDeviation(input_col='close', axis=axis, window_type=window_type).compute(binance_spot)
    if axis != 'cs':
        actual_btc = StandardDeviation(input_col='close', axis=axis, window_type=window_type).compute(btc_spot_prices)
    ...
    if axis == 'ts' and window_type == 'fixed':
        ...
        assert actual['std'].equals(binance_spot.close.groupby(level=1).transform('std'))  # values
        assert np.allclose(actual_btc['std'].mean(), btc_spot_prices.close.std())
        assert np.allclose(actual.loc[:, 'BTC', :]['std'], actual_btc['std'], equal_nan=True)
        ...
        assert np.allclose(actual.loc[:, 'BTC', :]['std'], actual_btc['std'], equal_nan=True)  # values
        assert isinstance(actual, pd.DataFrame)  # dtypes
        assert isinstance(actual_btc, pd.DataFrame)
        assert all(actual.dtypes == np.float64)
        assert all(actual_btc.dtypes == np.float64)
...
def test_quantile(binance_spot, btc_spot_prices, q, axis, window_type) -> None:
    ...
    """
    # get actual and expected
    actual = Quantile(input_col='close', q=q, axis=axis, window_type=window_type).compute(binance_spot)
    if axis != 'cs':
        actual_btc = Quantile(input_col='close', q=q, axis=axis, window_type=window_type).compute(btc_spot_prices)
...
def test_iqr(binance_spot, btc_spot_prices, axis, window_type) -> None:
    ...
    """
    # get actual and expected
    actual = IQR(input_col='close', axis=axis, window_type=window_type).compute(binance_spot)
    if axis != 'cs':
        actual_btc = IQR(input_col='close', axis=axis, window_type=window_type).compute(btc_spot_prices)
...
def test_mad(binance_spot, btc_spot_prices, axis, window_type) -> None:
    ...
    """
    # get actual and expected
    actual = MedianAbsoluteDeviation(input_col='close', axis=axis, window_type=window_type).compute(binance_spot)
    if axis != 'cs':
        actual_btc = MedianAbsoluteDeviation(input_col='close', axis=axis, window_type=window_type).compute(btc_spot_prices)
...
def test_variance(binance_spot, btc_spot_prices, axis, window_type) -> None:
    ...
    """
    # get actual and expected
    actual = Variance(input_col='close', axis=axis, window_type=window_type).compute(binance_spot)
    if axis != 'cs':
        actual_btc = Variance(input_col='close', axis=axis, window_type=window_type).compute(btc_spot_prices)
