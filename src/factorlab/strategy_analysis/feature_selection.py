...
class FeatureSelection:
        ...
        if self.quantize:
            ...
            if self.strategy == 'ts':
                self.features = Transform(self.features).quantize(bins=self.feature_bins, axis='ts',
                                                                ...
                                                                window_size=self.window_size)
                self.target = Transform(self.target).quantize(bins=self.target_bins, axis='ts', window_type=self.window_type,
                                                        window_size=self.window_size)
            # cross-sectional
            elif self.strategy == 'cs':
    ...
    def mutual_info(self) -> pd.DataFrame:
        ...
        # loop through factors, compute spearman rank corr
        for feat in self.features.columns:
            self.ranked_features.loc[feat, 'mutual_info'] = mutual_info_classif(self.features[[feat]], self.target.iloc[:, 0])
...
#
#         # ranked features
#         self.ranked_features = self.x.iloc[:, sorted_coef_idxs].copy()
#         self.ranked_features_list = self.ranked_features.columns.tolist()
...
#         self.ranked_features = self.features.loc[:, self.ranked_features.columns].copy()
